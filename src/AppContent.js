...
import WappEs from './es/components/wapp'
...
const AppContent = () => {
    ...
    useEffect(() => {
      // Extraer el idioma de la ruta y actualizar si es necesario
      const  hash = window.location.hash;
      const languageFromPath = hash.split('/')[1];
      if (languageFromPath && languageFromPath !== language) {
        changeLanguage(languageFromPath);
      }
    }, [language, changeLanguage]);

    // Elige componentes según el idioma actual
    ...
    return (
        <Router>
            <NavbarComponent height={10} data-aos-easing="ease" data-aos-duration="400" data-aos-delay="0" />
